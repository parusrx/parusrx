...
public class TpggServiceTests
{
    private readonly Mock<HttpMessageHandler> _httpMessageHandlerMock;
    private readonly TpggService _service;

    public TpggServiceTests()
    {
        ...
        _service = new TpggService(httpClient);
    }

    [Fact]
    public async Task ListAsync_ShouldReturnTpggResponse_WhenRequestIsSuccessful()
    {
        // Arrange
        var queryParameters = new Dictionary<string, string?> { { "key", "value" } };
        var expectedResponse = new ListResponse<Tpgg>();
