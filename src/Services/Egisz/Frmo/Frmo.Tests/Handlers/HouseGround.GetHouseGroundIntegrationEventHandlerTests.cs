...
public class GetHouseGroundIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IHouseGroundService> _serviceMock;
    private readonly Mock<ILogger<GetHouseGroundIntegrationEventHandler>> _loggerMock;

    public GetHouseGroundIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IHouseGroundService>();
        _loggerMock = new Mock<ILogger<GetHouseGroundIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        var response = new SingleResponse<HouseGround>()
        {
            RequestId = Guid.NewGuid().ToString(),
            Content = new HouseGround
            {
                HouseGroundId = Guid.NewGuid().ToString()
            }
        };
        ...
        var handler = new GetHouseGroundIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
