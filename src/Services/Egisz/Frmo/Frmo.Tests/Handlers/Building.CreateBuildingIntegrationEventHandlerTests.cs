...
public class CreateBuildingIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IBuildingService> _serviceMock;
    private readonly Mock<ILogger<CreateBuildingIntegrationEventHandler>> _loggerMock;

    public CreateBuildingIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IBuildingService>();
        _loggerMock = new Mock<ILogger<CreateBuildingIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Building>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateBuildingIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<Building>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Building>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
