...
public class CreateAirAmbulanceIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IAirAmbulanceService> _serviceMock;
    private readonly Mock<ILogger<CreateAirAmbulanceIntegrationEventHandler>> _loggerMock;

    public CreateAirAmbulanceIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IAirAmbulanceService>();
        _loggerMock = new Mock<ILogger<CreateAirAmbulanceIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<AirAmbulance>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateAirAmbulanceIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<AirAmbulance>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<AirAmbulance>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
