...
public class UpdateSiteIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<ISiteService> _serviceMock;
    private readonly Mock<ILogger<UpdateSiteIntegrationEventHandler>> _loggerMock;

    public UpdateSiteIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<ISiteService>();
        _loggerMock = new Mock<ILogger<UpdateSiteIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<Site>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new UpdateSiteIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.UpdateAsync(request.Parameters, It.IsAny<Site>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<Site>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
