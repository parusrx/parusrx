...
public class CreateTerritorialDepartIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<ITerritorialDepartService> _serviceMock;
    private readonly Mock<ILogger<CreateTerritorialDepartIntegrationEventHandler>> _loggerMock;

    public CreateTerritorialDepartIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<ITerritorialDepartService>();
        _loggerMock = new Mock<ILogger<CreateTerritorialDepartIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<TerritorialDepart>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateTerritorialDepartIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<TerritorialDepart>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<TerritorialDepart>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
