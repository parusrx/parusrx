...
public class CreateServiceInfoIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IServiceInfoService> _serviceMock;
    private readonly Mock<ILogger<CreateServiceInfoIntegrationEventHandler>> _loggerMock;

    public CreateServiceInfoIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IServiceInfoService>();
        _loggerMock = new Mock<ILogger<CreateServiceInfoIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<ServiceInfo>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateServiceInfoIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<ServiceInfo>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<ServiceInfo>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
