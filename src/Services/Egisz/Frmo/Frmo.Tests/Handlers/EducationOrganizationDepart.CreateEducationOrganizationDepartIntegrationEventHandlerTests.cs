...
public class CreateEducationOrganizationDepartIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IEducationOrganizationDepartService> _serviceMock;
    private readonly Mock<ILogger<CreateEducationOrganizationDepartIntegrationEventHandler>> _loggerMock;

    public CreateEducationOrganizationDepartIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IEducationOrganizationDepartService>();
        _loggerMock = new Mock<ILogger<CreateEducationOrganizationDepartIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<EducationOrganizationDepart>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateEducationOrganizationDepartIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<EducationOrganizationDepart>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<EducationOrganizationDepart>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
