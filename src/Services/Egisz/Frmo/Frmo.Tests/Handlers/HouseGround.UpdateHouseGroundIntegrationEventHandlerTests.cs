...
public class UpdateHouseGroundIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IHouseGroundService> _serviceMock;
    private readonly Mock<ILogger<UpdateHouseGroundIntegrationEventHandler>> _loggerMock;

    public UpdateHouseGroundIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IHouseGroundService>();
        _loggerMock = new Mock<ILogger<UpdateHouseGroundIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<HouseGround>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new UpdateHouseGroundIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.UpdateAsync(request.Parameters, It.IsAny<HouseGround>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<HouseGround>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
