...
public class CreateHouseGroundIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IHouseGroundService> _serviceMock;
    private readonly Mock<ILogger<CreateHouseGroundIntegrationEventHandler>> _loggerMock;

    public CreateHouseGroundIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IHouseGroundService>();
        _loggerMock = new Mock<ILogger<CreateHouseGroundIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<HouseGround>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateHouseGroundIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<HouseGround>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<HouseGround>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
