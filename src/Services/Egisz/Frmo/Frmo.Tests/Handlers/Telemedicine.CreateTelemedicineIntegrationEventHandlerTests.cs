...
public class CreateTelemedicineIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<ITelemedicineService> _serviceMock;
    private readonly Mock<ILogger<CreateTelemedicineIntegrationEventHandler>> _loggerMock;

    public CreateTelemedicineIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<ITelemedicineService>();
        _loggerMock = new Mock<ILogger<CreateTelemedicineIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Telemedicine>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateTelemedicineIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<Telemedicine>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Telemedicine>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
