...
public class UpdateEducationOrganizationDepartIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IEducationOrganizationDepartService> _serviceMock;
    private readonly Mock<ILogger<UpdateEducationOrganizationDepartIntegrationEventHandler>> _loggerMock;

    public UpdateEducationOrganizationDepartIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IEducationOrganizationDepartService>();
        _loggerMock = new Mock<ILogger<UpdateEducationOrganizationDepartIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<EducationOrganizationDepart>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new UpdateEducationOrganizationDepartIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.UpdateAsync(request.Parameters, It.IsAny<EducationOrganizationDepart>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<EducationOrganizationDepart>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
