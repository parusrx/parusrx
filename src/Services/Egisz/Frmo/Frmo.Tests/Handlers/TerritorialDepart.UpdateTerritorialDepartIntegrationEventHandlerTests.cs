...
public class UpdateTerritorialDepartIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<ITerritorialDepartService> _serviceMock;
    private readonly Mock<ILogger<UpdateTerritorialDepartIntegrationEventHandler>> _loggerMock;

    public UpdateTerritorialDepartIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<ITerritorialDepartService>();
        _loggerMock = new Mock<ILogger<UpdateTerritorialDepartIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<TerritorialDepart>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new UpdateTerritorialDepartIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.UpdateAsync(request.Parameters, It.IsAny<TerritorialDepart>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<TerritorialDepart>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
