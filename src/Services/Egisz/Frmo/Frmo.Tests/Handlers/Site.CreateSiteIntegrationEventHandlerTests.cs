...
public class CreateSiteIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<ISiteService> _serviceMock;
    private readonly Mock<ILogger<CreateSiteIntegrationEventHandler>> _loggerMock;

    public CreateSiteIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<ISiteService>();
        _loggerMock = new Mock<ILogger<CreateSiteIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Site>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateSiteIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<Site>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Site>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
