...
public class CreateEquipmentIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IEquipmentService> _serviceMock;
    private readonly Mock<ILogger<CreateEquipmentIntegrationEventHandler>> _loggerMock;

    public CreateEquipmentIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IEquipmentService>();
        _loggerMock = new Mock<ILogger<CreateEquipmentIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Equipment>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateEquipmentIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<Equipment>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Equipment>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
