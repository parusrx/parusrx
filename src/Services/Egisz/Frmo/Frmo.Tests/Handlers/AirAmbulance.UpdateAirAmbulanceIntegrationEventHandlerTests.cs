...
public class UpdateAirAmbulanceIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IAirAmbulanceService> _serviceMock;
    private readonly Mock<ILogger<UpdateAirAmbulanceIntegrationEventHandler>> _loggerMock;

    public UpdateAirAmbulanceIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IAirAmbulanceService>();
        _loggerMock = new Mock<ILogger<UpdateAirAmbulanceIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<AirAmbulance>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new UpdateAirAmbulanceIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.UpdateAsync(request.Parameters, It.IsAny<AirAmbulance>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.UpdateAsync(request.Parameters, It.IsAny<AirAmbulance>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
