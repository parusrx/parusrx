...
public sealed class GetAirAmbulanceIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IAirAmbulanceService> _serviceMock;
    private readonly Mock<ILogger<GetAirAmbulanceIntegrationEventHandler>> _loggerMock;

    public GetAirAmbulanceIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IAirAmbulanceService>();
        _loggerMock = new Mock<ILogger<GetAirAmbulanceIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        var response = new SingleResponse<AirAmbulance>()
        {
            RequestId = Guid.NewGuid().ToString(),
            Content = new AirAmbulance()
            {
                AirAmbulanceId = Guid.NewGuid().ToString()
            }
        };
        ...
        var handler = new GetAirAmbulanceIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
