...
public sealed class UpdateAirAmbulanceIntegrationEventHandler(
    IParusRxStore store,
    IAirAmbulanceService service,
    ILogger<UpdateAirAmbulanceIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<AirAmbulance>>(data)
