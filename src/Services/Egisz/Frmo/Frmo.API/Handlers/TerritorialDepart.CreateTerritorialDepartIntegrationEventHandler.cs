...
public sealed class CreateTerritorialDepartIntegrationEventHandler(
    IParusRxStore store,
    ITerritorialDepartService service,
    ILogger<CreateTerritorialDepartIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<TerritorialDepart>>(data)
