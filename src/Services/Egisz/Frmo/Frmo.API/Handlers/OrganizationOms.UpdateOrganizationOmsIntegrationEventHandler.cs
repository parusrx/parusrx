...
public sealed class UpdateOrganizationOmsIntegrationEventHandler(
    IParusRxStore store,
    IOrganizationOmsService service,
    ILogger<UpdateOrganizationOmsIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<OrganizationOms>>(data)
