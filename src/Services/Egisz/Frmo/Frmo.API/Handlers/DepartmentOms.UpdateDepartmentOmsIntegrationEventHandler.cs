...
public sealed class UpdateDepartmentOmsIntegrationEventHandler(
    IParusRxStore store,
    IDepartmentOmsService service,
    ILogger<UpdateDepartmentOmsIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<DepartOms>>(data)
