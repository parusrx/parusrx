...
public sealed class DeleteEducationOrganizationDepartIntegrationEventHandler(IParusRxStore store, IEducationOrganizationDepartService service, ILogger<DeleteEducationOrganizationDepartIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var response = await service.DeleteAsync(request.Parameters, cancellationToken);
