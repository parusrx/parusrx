...
public sealed class CreateMobileDepartIntegrationEventHandler(
    IParusRxStore store,
    IMobileDepartService service,
    ILogger<CreateMobileDepartIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<MobileDepart>>(data)
