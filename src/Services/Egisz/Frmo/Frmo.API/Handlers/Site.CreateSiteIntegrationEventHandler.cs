...
public sealed class CreateSiteIntegrationEventHandler(IParusRxStore store, ISiteService service, ILogger<CreateSiteIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<Site>>(data)
