...
public sealed class CreateServiceInfoIntegrationEventHandler(IParusRxStore store, IServiceInfoService service, ILogger<CreateServiceInfoIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<ServiceInfo>>(data)
