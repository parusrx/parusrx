...
public sealed class UpdateEquipmentIntegrationEventHandler(
    IParusRxStore store,
    IEquipmentService service,
    ILogger<UpdateEquipmentIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<Equipment>>(data)
