...
public sealed class CreateHouseGroundIntegrationEventHandler(
    IParusRxStore store,
    IHouseGroundService service,
    ILogger<CreateHouseGroundIntegrationEventHandler> logger) : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<HouseGround>>(data)
