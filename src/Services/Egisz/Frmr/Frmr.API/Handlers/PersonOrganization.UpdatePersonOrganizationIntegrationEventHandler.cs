...
public sealed class UpdatePersonOrganizationIntegrationEventHandler(IParusRxStore store, IPersonOrganizationService service, ILogger<UpdatePersonOrganizationIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<PersonOrganization>>(data)
