...
public sealed class UpdateMilitaryServiceIntegrationEventHandler(IParusRxStore store, IMilitaryClient service, ILogger<UpdateMilitaryServiceIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<MilitaryService>>(data)
