...
public sealed class UpdateEducationProfIntegrationEventHandler(IParusRxStore store, IEducationProfService service, ILogger<UpdateEducationProfIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<EducationProf>>(data)
