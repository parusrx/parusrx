...
public sealed class CreateEducationCommonIntegrationEventHandler(IParusRxStore store, IEducationCommonService service, ILogger<CreateEducationCommonIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<EducationCommon>>(data)
