...
public class CreatePersonIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IPersonService> _serviceMock;
    private readonly Mock<ILogger<CreatePersonIntegrationEventHandler>> _loggerMock;

    public CreatePersonIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IPersonService>();
        _loggerMock = new Mock<ILogger<CreatePersonIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Person>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreatePersonIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<Person>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<Person>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
