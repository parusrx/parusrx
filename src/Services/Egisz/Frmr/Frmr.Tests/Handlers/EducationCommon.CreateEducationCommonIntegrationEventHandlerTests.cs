...
namespace ParusRx.Frmr.Tests.Handlers;

public class CreateEducationCommonIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IEducationCommonService> _serviceMock;
    private readonly Mock<ILogger<CreateEducationCommonIntegrationEventHandler>> _loggerMock;

    public CreateEducationCommonIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IEducationCommonService>();
        _loggerMock = new Mock<ILogger<CreateEducationCommonIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        DefaultRequest<EducationCommon> request = new()
        {
            Parameters = new()
            {
                { "key", "value" }
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<EducationCommon>(), It.IsAny<CancellationToken>())).ReturnsAsync(response);

        var handler = new CreateEducationCommonIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
        ...
        _serviceMock.Verify(s => s.CreateAsync(request.Parameters, It.IsAny<EducationCommon>(), It.IsAny<CancellationToken>()), Times.Once());
    }
    ...
    public async Task HandleAsync_WithException_SaveErrorAndLogError()
    {
        ...
        _serviceMock.Setup(s => s.CreateAsync(request.Parameters, It.IsAny<EducationCommon>(), It.IsAny<CancellationToken>())).ThrowsAsync(new Exception());
