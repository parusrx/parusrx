...
public sealed class DocumentService(HttpClient httpClient) : IDocumentService
{
    ...
    /// </inheritdoc>
    public async ValueTask<SendToSigningResponse?> SendToSigningAsync(SendToSigningRequest request, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(request);

        string uri = $"{request.Authorization.Url}/api/v1/clients/{request.Authorization.ClientId}/documentGroups/sendToSigning";
        httpClient.DefaultRequestHeaders.Add("User-Api-Token", request.Authorization.ApiToken);
        ...
        return await response.Content.ReadFromJsonAsync<SendToSigningResponse>(cancellationToken);
    }
}
