...
public class GetByOidDepartmentIntegrationEventHandlerTests
{
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        _serviceMock.Setup(s => s.GetByOidAsync(request.Parameters.DepartOid, request.Parameters.Oid, It.IsAny<CancellationToken>())).ReturnsAsync(response);
        ...
        _serviceMock.Verify(s => s.GetByOidAsync(request.Parameters.DepartOid, request.Parameters.Oid, It.IsAny<CancellationToken>()), Times.Once());
    }
