...
public class GetFullPersonIntegrationEventHandlerTests
{
    private readonly Mock<IParusRxStore> _storeMock;
    private readonly Mock<IFullPersonService> _serviceMock;
    private readonly Mock<ILogger<GetFullPersonIntegrationEventHandler>> _loggerMock;

    public GetFullPersonIntegrationEventHandlerTests()
    {
        _storeMock = new Mock<IParusRxStore>();
        _serviceMock = new Mock<IFullPersonService>();
        _loggerMock = new Mock<ILogger<GetFullPersonIntegrationEventHandler>>();
    }
    ...
    public async Task HandleAsync_WithValidRequest_SaveDataResponse()
    {
        ...
        var response = new SingleResponse<FullPerson>()
        {
            RequestId = Guid.NewGuid().ToString(),
            Content = new FullPerson()
            {
                Mr = true,
                FirstName = "John",
                LastName = "Doe",
        ...
        var handler = new GetFullPersonIntegrationEventHandler(_storeMock.Object, _serviceMock.Object, _loggerMock.Object);
