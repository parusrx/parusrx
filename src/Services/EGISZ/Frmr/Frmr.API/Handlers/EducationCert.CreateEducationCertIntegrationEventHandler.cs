...
public sealed class CreateEducationCertIntegrationEventHandler(IParusRxStore store, IEducationCertService service, ILogger<CreateEducationCertIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<EducationCert>>(data)
