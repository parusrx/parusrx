...
public sealed class CreatePersonNominationIntegrationEventHandler(IParusRxStore store, IPersonNominationService service, ILogger<CreatePersonNominationIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<PersonNomination>>(data)
