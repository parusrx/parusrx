...
public sealed class UpdatePersonAccreditationIntegrationEventHandler(IParusRxStore store, IPersonAccreditationService service, ILogger<UpdatePersonAccreditationIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<PersonAccreditation>>(data)
