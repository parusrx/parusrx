...
public sealed class CreateEducationPostgraduateIntegrationEventHandler(IParusRxStore store, IEducationPostgraduateService service, ILogger<CreateEducationPostgraduateIntegrationEventHandler> logger)
    : IIntegrationEventHandler<MqIntegrationEvent>
{
    public async Task HandleAsync(MqIntegrationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var request = XmlSerializerUtility.Deserialize<DefaultRequest<EducationPostgraduate>>(data)
